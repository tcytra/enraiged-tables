...
use Enraiged\Enums\TemplateSources;
use Enraiged\Builders\Traits\ParameterLoader;
...
trait BuilderConstructor
{
    ...
    public function __construct(Request $request, array $parameters = [])
    {
        ...
        if ($this->source === TemplateSources::Database->value) {
        }

        if ($this->source === TemplateSources::Filesystem->value) {
            if (!$this->template) {
            ...
            if (File::mimeType($this->template) === FileTypes::JSON->value) {
                $this->load(json_decode(file_get_contents($this->template), true));
            }
            if (File::mimeType($this->template) === FileTypes::PHP->value) {
