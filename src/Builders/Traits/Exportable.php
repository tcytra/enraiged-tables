<?php

namespace Enraiged\Tables\Builders\Traits;

//Enraiged\Builders\UriBuilder;
use Illuminate\Contracts\Queue\ShouldQueue;

trait Exportable
{
    /** @var  int  The exportable chunk size. */
    protected int $chunksize;

    /** @var  array  The exportable parameters. */
    protected $exportable;

     /** @var  string  The exported file location. */
    protected $exportpath;

     /** @var  string  The human readable filename. */
    protected $filename;

     /** @var  string  The exported file type. */
    protected $filetype;

    /** @var  string  The hashed (storage) filename. */
    protected $hashname;

    /**
     *  Return the export configuration.
     *
     *  @return array|null
     */
    public function exportableConfiguration()
    {
        if ($this->get('exportable') && (is_null($this->model) || $this->user->can('export', $this->model))) {
            $exportable = $this->get('exportable');

            $route = !is_array($exportable['route'])
                ? ['name' => $exportable['route']]
                : $exportable['route'];

            if (!key_exists('name', $route)) {
                $route['name'] = preg_replace('/\.$/', '', $this->prefix).'.export';
            }

            if (!key_exists('method', $route)) {
                $route['method'] = 'post';
            }

            if (!key_exists('params', $route)) {
                $route['params'] = $this->request()
                    ->route()
                    ->parameters();
            }

            if (!key_exists('url', $route)) {
                $route['url'] = route($route['name'], $route['params'], config('enraiged.app.absolute_uris'));
            }

            return collect($this->get('exportable'))
                ->merge(['route' => $route])
                ->toArray();
        }

        return null;
    }

    /**
     *  Return the data sources for the searchable columns.
     *
     *  @return array
     */
    public function exportableColumns(): array
    {
        return collect($this->columns)
            ->filter(fn ($column)
                => (!key_exists('exportable', $column) || $column['exportable'] !== false)
                    && key_exists('label', $column)
                    && $this->assertSecure($column))
            ->transform(fn ($column) => $column['label'])
            ->toArray();
    }

    /**
     *  Return the human readable filename with filetype.
     *
     *  @return string
     */
    public function exportableFilename()
    {
        $filename = key_exists('name', $this->exportable)
            ? $this->exportable['name']
            : $this->id;
        $filetype = strtolower($this->request->get('export'));
        $datetime = date('Y-m-d His');

        return "{$filename} {$datetime}.{$filetype}";
    }

    /**
     *  Return the exported file destination location.
     *
     *  @return string
     */
    public function exportableLocation()
    {
        $storage = config('enraiged.tables.storage');
        $filetype = strtolower($this->request->get('export'));
        $hashname = md5($this->request->get('export').microtime());
        $exportpath = "{$storage}/{$hashname}.{$filetype}";

        return $exportpath;
    }

    /**
     *  Return the exporter service object.
     *
     *  @return object
     */
    public function exporter()
    {
        if (gettype($this->exporter === 'string')) {
            $this->exporter = $this->exporter::from($this);
        }

        return $this->exporter;
    }

    /**
     *  Determine if the table export is handled by the queue.
     *
     *  @return bool
     */
    public function isAutoDownload(): bool
    {
        $exportable = (object) $this->get('exportable');

        return $exportable
            && property_exists($exportable, 'autodownload')
            && $exportable->autodownload === true;
    }

    /**
     *  Determine if the table export is handled by the queue.
     *
     *  @return bool
     */
    public function isQueuedExport(): bool
    {
        $exporter = $this->exporter;

        return gettype($exporter) === 'object' && !$this->isAutoDownload()
            && $exporter instanceof ShouldQueue
            && config('queue.default') !== 'sync';
    }

    /**
     *  Determine if the current process is running from within the queue.
     *
     *  @return bool
     */
    private function isQueued(): bool
    {
        return app()->resolved(\Illuminate\Queue\CallQueuedHandler::class) == 1;
    }
}
